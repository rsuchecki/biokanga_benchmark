#!/bin/bash
#BSUB -J hisat2-align-tune_job            # LSF job name
#BSUB -o hisat2-align-tune_job.%J.out     # Name of the job output file
#BSUB -e hisat2-align-tune_job.%J.error   # Name of the job error file
...
MAX_SOFTCLIPPING_PENALITY=${12}
MIN_SOFTCLIPPING_PENALITY=${13}

# define output path
OUTPUT_INDEX_PATH=$OUTPUT_ROOT_PATH/$TOOL/index/$GENOME_NAME
OUTPUT_ALIGNMENT_PATH=$OUTPUT_ROOT_PATH/$TOOL/alignment/$LIBRARY-tune/tune-$MODE-$NUM_MISMATCH-$SEED_LENGTH-$SEED_INTERVAL-$SEED_EXTENSION-$RE_SEED-$PENALITY_NONCANONICAL-$MAX_MISMATCH_PENALITY-$MIN_MISMATCH_PENALITY-$MAX_SOFTCLIPPING_PENALITY-$MIN_SOFTCLIPPING_PENALITY
mkdir -p $OUTPUT_ALIGNMENT_PATH

if [ $MODE != "local" ]
then
        # run hisat2 (in end-to-end mode)
